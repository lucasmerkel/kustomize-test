...
exports.proxyHostAndPort = exports.addProxyConfigurationOnPrem = void 0;
 ...
 */
async function addProxyConfigurationOnPrem(destination, jwt) {
    if (destination.authentication === 'PrincipalPropagation' &&
        !(0, jwt_1.isUserToken)(jwt)) {
        throw new Error('For principal propagation a user JWT is needed.');
    }
    const proxyConfiguration = {
        ...proxyHostAndPort(),
        headers: {
            ...(await proxyHeaders(destination.authentication, jwt === null || jwt === void 0 ? void 0 : jwt.encoded))
        }
    };
...
function readConnectivityServiceBinding() {
    const serviceBindings = (0, environment_accessor_1.getServiceList)('connectivity');
...
async function proxyHeaders(authenticationType, jwt) {
    const proxyAuthHeader = await proxyAuthorizationHeader(jwt);
    const sapConnectivityHeader = sapConnectivityAuthenticationHeader(authenticationType, jwt);
    return {
        ...proxyAuthHeader,
